...
public class ModelLayerHelper {
	...
	public static void registerLayerDefinitions(EntityRenderersEvent.RegisterLayerDefinitions event) {
		...
		event.registerLayerDefinition(ClientHandler.FROG_NOSE, FrogNoseModel::createBodyLayer);
	}
	...
	public static void registerAdditionalLayers(EntityRenderersEvent.AddLayers event) {
		...
		if (event.getRenderer(EntityType.FROG) instanceof FrogRenderer frogRenderer) {
			frogRenderer.addLayer(new FrogNoseLayer<>(frogRenderer, event.getEntityModels()));
		}
	}
}
